...
var rawModels = ModelManager.loadPartedRawModel(Resources.manager(), Resources.idRelative("mtr:ktm23/ktm23-spb.obj"), null);
...
function render(ctx, state, train) {
	...
	for (i = 0; i < train.trainCars(); i++) {
		...
		*/
		...
		ctx.drawCarModel(models["svet"], i, matrices);
		ctx.drawCarModel(models["bg1"], i, matrices);
		ctx.drawCarModel(models["bg2"], i, matrices);
		ctx.drawCarModel(models["gabarit"], i, matrices);

		//рендер дверей определен нижк=е
		renderDoorWithChild(matrices, ctx,  models["k1"],  models["d1"], null, models["d1_1"], -firstAngle, -secondAngle, -1.140, 4.879, -1.177, 5.171, i);

		renderDoorWithChild(matrices, ctx, models["k2"], models["d2"], null, models["d2_2"], firstAngle, secondAngle, -1.140, 1.894, -1.177, 1.647, i);
		renderDoorWithChild(matrices, ctx, models["k3"], models["d3"], null, models["d3_3"], -firstAngle, -secondAngle, -1.140, 0.5751, -1.177, 0.8252, i);

		renderDoorWithChild(matrices, ctx, models["k4"], models["d4"], null, models["d4_4"], firstAngle, secondAngle, -1.140, -0.9223, -1.177, -1.163, i);
		renderDoorWithChild(matrices, ctx, models["k5"], models["d5"], null, models["d5_5"], -firstAngle, -secondAngle, -1.140, -2.240, -1.177, -1.999, i);

		renderDoorWithChild(matrices, ctx, models["k6"], models["d6"], null, models["d6_6"], -firstAngle, -secondAngle, -1.140, -6.117, -1.177, -5.834, i);
	}
...
function uploadPartedModels(rawModels) {
	...
	for (it = rawModels.entrySet().iterator(); it.hasNext();) {
		...
		if (jsStringKey == "svet") entry.getValue().setAllRenderType("light");
