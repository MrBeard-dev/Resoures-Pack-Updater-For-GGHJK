...
var rawModels = ModelManager.loadPartedRawModel(Resources.manager(), Resources.idRelative("mtr:lm99k/lm99.obj"), null);
...
function render(ctx, state, train) {
	...
	for (i = 0; i < train.trainCars(); i++) {
		...
		*/
		...
		ctx.drawCarModel(models["int"], i, matrices);
		ctx.drawCarModel(models["disp"], i, matrices);
		ctx.drawCarModel(models["bog1"], i, matrices);
		ctx.drawCarModel(models["bog2"], i, matrices);
		ctx.drawCarModel(models["fara"], i, matrices);
		ctx.drawCarModel(models["lamp"], i, matrices);
		ctx.drawCarModel(models["pant_low"], i, matrices);
		ctx.drawCarModel(models["pant_hight"], i, matrices);
		ctx.drawCarModel(models["door_m"], i, matrices);
		ctx.drawCarModel(models["door_ml"], i, matrices);

		//рендер дверей определен нижк=е
		renderDoorWithChild(matrices, ctx,  models["dr1"],  null, null, null, firstAngle, secondAngle, -1.132, 1.966, -1.132, 1.718, i);
		renderDoorWithChild(matrices, ctx,  null,  models["d1"], null, models["d1_1"], firstAngle, secondAngle, -1.165, 1.927, -1.165, 1.718, i);
		renderDoorWithChild(matrices, ctx,  models["dr2"],  null, null, null, -firstAngle, -secondAngle, -1.133, 0.7193, -1.133, 0.9633, i);
		renderDoorWithChild(matrices, ctx,  null,  models["d2"], null, models["d2_2"], -firstAngle, -secondAngle, -1.166, 0.7689, -1.166, 0.9657, i);
		
		renderDoorWithChild(matrices, ctx, models["dr3"], null, null, null, firstAngle, secondAngle, -1.134, -0.8401, -1.134, -1.086, i);
		renderDoorWithChild(matrices, ctx, null, models["d3"], null, models["d3_3"], firstAngle, secondAngle, -1.165, -0.8902, -1.165, -1.087, i);
		renderDoorWithChild(matrices, ctx, models["dr4"], null, null, null, -firstAngle, -secondAngle, -1.133, -2.087, -1.133, -1.841, i);
		renderDoorWithChild(matrices, ctx, null, models["d4"], null, models["d4_4"], -firstAngle, -secondAngle, -1.165, -2.038, -1.165, -1.842, i);

		renderDoorWithChild(matrices, ctx, models["dr5"], null, null, null, firstAngle, secondAngle, -1.092, -5.659, -1.046, -5.901, i);
		renderDoorWithChild(matrices, ctx, null, models["d5"], null, models["d5_5"], firstAngle, secondAngle, -1.114, -5.715, -1.077, -5.904, i);
		renderDoorWithChild(matrices, ctx, models["dr6"], null, null, null, -firstAngle, -secondAngle, -0.8492, -6.883, -0.8984, -6.642, i);
		renderDoorWithChild(matrices, ctx, null, models["d6"], null, models["d6_6"], -firstAngle, -secondAngle, -0.8919, -6.840, -0.9295, -6.640, i);
	}
...
function uploadPartedModels(rawModels) {
	...
	for (it = rawModels.entrySet().iterator(); it.hasNext();) {
		...
		if (jsStringKey == "int") entry.getValue().setAllRenderType("interior"); //включение интерьера
		...
		if (jsStringKey == "int") entry.getValue().setAllRenderType("interior");
		if (jsStringKey == "lamp") entry.getValue().setAllRenderType("light");
